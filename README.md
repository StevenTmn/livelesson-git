...
- `git commit`: open a text editor to write commit message
	- `git commit -m "MESSAGE`: writes MESSAGE as a comit without a text editor

- `git log`: shows the log (history) of our commits
	- `git log --oneline`: shows the shorter oneline commit 
- `git diff`: compare current uncommited state with the last known git state
	- `git diff --staged`: runs git diff between the staging area and last known state
- `git diff HEAD~<NUMBER>`: comapres HEAD with commit <NUMBER> ago (relative)
...
- `git switch -c <NAME>`: create and move to branch <NAME> in 1 command
...
- `git stash` or `git commit`: to save work before moving branches
	...
	- `git stash apply`: apply your last stashed commtit
...
- `git fetch`: update your git log without making any changes to your files
	- `git fetch --prune`: update your log and also remove deleted remote branch

- `git push -f <WHERE> <WHAT>`: force push to the remote <WHERE> the branch <WHAT>
	- `git push --force-with-lease <WHERE> <WHERE> <WHAT>`: more mindful of collaborators
